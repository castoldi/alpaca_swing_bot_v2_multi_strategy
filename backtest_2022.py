"""Full-year 2022 backtest — all registered strategies on real market data.

Covers the 2022 rate-hike bear market (SPX -25.4%, 2022-01-03 to 2022-10-12)
cataloged in docs/bear-markets-and-crashes.md.

Usage:
    python backtest_2022.py                         # all strategies
    python backtest_2022.py --strategy breakout
"""
...
OUTPUT_PATH = ROOT / "reports" / "backtest_2022.html"
...
def run_full_backtest() -> int:
    log.info("=== 2022 Multi-Strategy Backtest (V2) ===")
    ...
    for strategy in strategies_to_run:
        ...
        bt_id = db_mod.start_backtest_run(2022, strat_name, strategy.timeframe)
        _, stats, per_ticker = run_strategy_year(
            ticker_data, strategy, 2022, PARAMS
        )
    ...
    html = html.replace("2025 Backtest", "2022 Backtest")
    OUTPUT_PATH.write_text(html, encoding="utf-8")

    log.info("=" * 50)
    log.info("2022 backtest complete. Best strategy: %s ($%.2f)", overall_best, best_pnl)
...
def run_single(strategy_name: str) -> int:
    ...
    log.info("Single-strategy backtest: %s (2022)", strat.name)
    ...
    bt_id = db_mod.start_backtest_run(2022, strat.name, strat.timeframe)
    _, stats, _ = run_strategy_year(ticker_data, strat, 2022, PARAMS)
    dd = stats["max_drawdown_pct"]
    pf = stats["profit_factor"]
    db_mod.finish_backtest_run(bt_id, stats["trades"], stats["win_rate"],
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="2022 multi-strategy backtest")
    parser.add_argument("--strategy", type=str, help="Run a single strategy by name")
    args = parser.parse_args()
    import runtime
    with runtime.tracked("backtest_2022", {"strategy": args.strategy or "all"}):
        if args.strategy:
