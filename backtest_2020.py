"""Full-year 2020 backtest — all registered strategies on real market data.

Covers the COVID crash (SPX -33.9%, 2020-02-19 to 2020-03-23) cataloged in
docs/bear-markets-and-crashes.md. Within Alpaca's ~2016+ data range, unlike
the 2000 and 2007-2009 bears which can only be studied at the index level.

Usage:
    python backtest_2020.py                         # all strategies
    python backtest_2020.py --strategy breakout
"""
...
OUTPUT_PATH = ROOT / "reports" / "backtest_2020.html"
...
def run_full_backtest() -> int:
    log.info("=== 2020 Multi-Strategy Backtest (V2) ===")
    ...
    for strategy in strategies_to_run:
        ...
        bt_id = db_mod.start_backtest_run(2020, strat_name, strategy.timeframe)
        _, stats, per_ticker = run_strategy_year(
            ticker_data, strategy, 2020, PARAMS
        )
    ...
    html = html.replace("2025 Backtest", "2020 Backtest")
    OUTPUT_PATH.write_text(html, encoding="utf-8")

    log.info("=" * 50)
    log.info("2020 backtest complete. Best strategy: %s ($%.2f)", overall_best, best_pnl)
...
def run_single(strategy_name: str) -> int:
    ...
    log.info("Single-strategy backtest: %s (2020)", strat.name)
    ...
    bt_id = db_mod.start_backtest_run(2020, strat.name, strat.timeframe)
    _, stats, _ = run_strategy_year(ticker_data, strat, 2020, PARAMS)
    dd = stats["max_drawdown_pct"]
    pf = stats["profit_factor"]
    db_mod.finish_backtest_run(bt_id, stats["trades"], stats["win_rate"],
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="2020 multi-strategy backtest")
    parser.add_argument("--strategy", type=str, help="Run a single strategy by name")
    args = parser.parse_args()
    import runtime
    with runtime.tracked("backtest_2020", {"strategy": args.strategy or "all"}):
        if args.strategy:
